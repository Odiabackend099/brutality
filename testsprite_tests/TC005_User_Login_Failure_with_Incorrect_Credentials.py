...
async def run_test():
    ...
    try:
        ...
        # Assertion: Ensure no session cookies are set and user remains on login page
        cookies = await context.cookies()
        assert not any(cookie['name'] == 'session' for cookie in cookies), 'Session cookie should not be set on failed login'
        assert 'login' in frame.url or 'signin' in frame.url, 'User is not on the login page after failed login'
