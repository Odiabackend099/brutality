...
async def run_test():
    ...
    try:
        ...
        # Click on 'Log In' to access the dashboard for AI Agent Management.
        ...
        # Try to use 'Sign in with Google' button to login alternatively.
        frame = context.pages[-1]
        ...
        # Input the registered email address and click 'Send Reset Link' button to initiate password reset.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/form/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('testuser@example.com')
        ...
        # Click 'Back to Sign In' to return to login page after password reset initiation.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click on 'Sign up' link to create a new account for testing AI voice agent management if no valid credentials available.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/div[3]/p/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Fill in Full Name, Company (optional), Email Address, and Password fields, then click 'Create Account' button.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/form/div/div/input').nth(0)
